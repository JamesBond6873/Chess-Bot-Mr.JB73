    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        int Negamax(int alpha, int beta, int intDepth, int ply)
        {
            ...
            for (int i = 0; i < moves.Length; i++)
            {
                ...
                //MVVLVA
                else if (moves[i].IsCapture) moveScores[i] -= 100 * (int)moves[i].CapturePieceType - (int)moves[i].MovePieceType;

                //Killer Moves
                else if (moves[i] == killers[ply]) moveScores[i] -= 900_000;
            ...
            // (Check/Stale)mate
            if (!qsearch && moves.Length == 0) return board.IsInCheck() ? -99999 + ply : 0;
